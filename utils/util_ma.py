...
def stable_batch_kernel_SV_mode(batch, img_H=250, img_W=250, divide_H=1, divide_W=1, sv_mode=0, l=21, sig=2.6, sig1=2.6,
    ...
    batch_kernel = np.zeros((batch, img_H * img_W, l, l))  # within each patch has the same kernel
    ...
    for ibatch in range(batch):
        block_H = math.ceil(img_H / divide_H)  # patch pos
...
def random_batch_noise(batch, high, rate_cln=1.0):
    # noise_level = np.random.uniform(size=(batch, 1)) * high
    # noise_mask = np.random.uniform(size=(batch, 1))
    noise_level = torch.rand(size=(batch, 1)) * high
    noise_mask = torch.rand(size=(batch, 1))
    noise_mask[noise_mask < rate_cln] = 0
    noise_mask[noise_mask >= rate_cln] = 1
    return noise_level * noise_mask

def random_batch_gl_noise(batch, high, rate_cln=1.0):
    noise_level = np.random.uniform(size=(1, 1)) * high
    noise_level = np.ones((batch, 1)) * noise_level
    noise_mask = np.random.uniform(size=(batch, 1))
    noise_mask[noise_mask < rate_cln] = 0
    noise_mask[noise_mask >= rate_cln] = 1
    return noise_level * noise_mask
...
# def b_GaussianNoising(tensor, sigma, mean=0.0, noise_size=None, min=0.0, max=255.0):
...
def b_GaussianNoising(tensor, sigma, mean=0.0, noise_size=None, min=0.0, max=255.0):
    ...
    # noise = torch.mul(sigma.new_tensor(np.random.normal(loc=mean, scale=1.0, size=size)),
    #                   sigma.view(sigma.size() + (1, 1)))
    noise = torch.mul(torch.randn(size, device=sigma.device),
                      sigma.view(sigma.size() + (1, 1)))
    return torch.clamp(noise + tensor, min=min, max=max)

def bp_GaussianNoising(tensor, sigma, mean=0.0, divide_P=1,
                       noise_size=None, min=0.0, max=255.0):
    """
    sigma : (B*N*block_H*block_HW, 1)
    tensor : (BN, C, H, W)
    """
    bn, c, h, w = tensor.shape
    BNBB, _ = sigma.size()
    if noise_size is None:
        size = torch.Size((BNBB, c, divide_P, divide_P))
    else:
        size = noise_size  # (B* C, K^2, H, W)
    is_pad = False
    if h % divide_P != 0 or w % divide_P != 0:
        is_pad = True
        if h % divide_P != 0:
            factor_h = h // divide_P + 1
        else:
            factor_h = h // divide_P
        if w % divide_P != 0:
            factor_w = w // divide_P + 1
        else:
            factor_w = w // divide_P
        pad_size_h = factor_h * divide_P - h
        pad_size_w = factor_w * divide_P - w
        tensor = torch.nn.functional.pad(tensor, [0, pad_size_w, 0, pad_size_h], mode='reflect')
    bn, c, hp, wp = tensor.shape
    tensor_p = F.unfold(tensor, kernel_size=divide_P, stride=divide_P)  # (BN1, CK^2, MN)
    tensor_p = tensor_p.transpose(1, 2).contiguous()
    tensor_p = tensor_p.view(BNBB, c, divide_P, divide_P)
    noise = torch.mul(sigma.new_tensor(np.random.normal(loc=mean, scale=1.0, size=size)),
                      sigma.view(sigma.size() + (1, 1)))
    tensor_p = torch.clamp(noise + tensor_p, min=min, max=max)  # (BNBB, c, divide_P, divide_P)
    tensor_f = tensor_p.view(bn, -1, c*divide_P*divide_P)
    tensor_f = tensor_f.transpose(1, 2).contiguous()  # *(bn, ckk, mn)

    out = torch.nn.functional.fold(tensor_f, output_size=(hp, wp), kernel_size=divide_P,
                                   stride=divide_P).view(bn, c, hp, wp)
    if is_pad:
        out = out[:, :, 0:h, 0:w]
    return out
...
class SRMDPreprocessing(object):
    def __init__(self, scale, random, sample_mode='bicubic',
                 kernel_size=21, noise=0.0, add_jpeg=False, device=torch.device('cuda'), sig=2.6,
                 sig1=2.6, sig2=2.6, theta=0,
                 sig_min=0.2, sig_max=4.0, blur_type='iso_gaussian', rate_cln=0.2, noise_high=0.05882, jpeg_low=75,
                 is_training=False, sv_mode=0, noise_var=False,
                 divide_H=40, divide_W=40):

        self.device = device
        self.l = kernel_size
        self.sample_mode = sample_mode
        self.noise = noise
        self.noise_high = noise
        self.jpeg = add_jpeg
        ...
        self.rate_iso = 1 if blur_type=='iso_gaussian' else 0
        self.is_training = is_training
        self.sv_mode = sv_mode
        self.noise_var = noise_var

        if self.sv_mode == 0:  # spatial-variant
            self.blur = BatchBlur(l=kernel_size, padmode='replication')
            self.kernel_gen = BatchSRKernel(l=kernel_size, sig=sig, sig1=sig1, sig2=sig2, theta=theta,
                                            sig_min=sig_min, sig_max=sig_max, rate_iso=self.rate_iso, scale=scale)
        else:  # spatial-invariant
            self.blur = BatchBlur_SV(l=kernel_size, padmode='replication')
            self.kernel_gen = BatchSRKernel_SV(l=kernel_size, sig=sig, sig1=sig1, sig2=sig2, theta=theta,
                                               sig_min=sig_min, sig_max=sig_max, rate_iso=self.rate_iso, scale=scale,
                                               divide_H=divide_H, divide_W=divide_W, sv_mode=sv_mode)
        if sample_mode == 'bicubic':  # TODO
            self.sample = BatchSubsample(scale=scale)
        elif sample_mode == 's-fold':
            self.sample = BatchBicubic(scale=scale)


    def __call__(self, hr_tensor, kernel=False):
        B, N, C, H, W = hr_tensor.size()
        hr = hr_tensor.view(-1, C, H, W)
        # generate kernel and blur
        # if self.sv_mode == 0 and self.rate_iso == 1 and self.kernel_gen.sig == 0:  # si, iso, no blur
        #     b_kernels = None
        #     b_sigmas = None
        #     lr = hr
        # else:
        if self.sv_mode == 0:
            if self.rate_iso == 1 and self.kernel_gen.sig == 0:
                b_kernels, b_sigmas = torch.zeros(B, self.l, self.l), torch.zeros(B, C)
                for i in range(B):
                    b_kernels[i][self.l // 2][self.l // 2] = 1
            else:
        ...
        lr = self.blur(hr.view(B, -1, H, W), b_kernels)
        lr = lr.view(-1, C, H, W)
        ...
        if self.noise != 0:
            # torch.randn()
            if self.noise_var is True:
                divide_H, divide_W = 20, 20  # test, the image is not that small why 10?
                _, _, lh, lw = lr_n.shape
                block_H = math.ceil(lh / divide_H)  # patch pos
                block_W = math.ceil(lw / divide_W)
                Noise_level = torch.FloatTensor(random_batch_noise(B*N*block_H*block_W, self.noise_high, self.rate_cln)).to(
                    self.device)   # local noise
                # Noise_level = torch.FloatTensor(
                #     random_batch_gl_noise(B*N*block_H*block_W, self.noise_high, self.rate_cln)).to(
                #     self.device)
                lr_n = bp_GaussianNoising(lr_n, Noise_level, divide_P=divide_H)
            else:
                if self.is_training:
                    Noise_level = torch.FloatTensor(random_batch_noise(B, self.noise_high, self.rate_cln)).to(
                        self.device)
                    Noise_level = Noise_level.repeat(1, N)
                    Noise_level = Noise_level.view(-1, 1)
                else:
                    Noise_level = (torch.ones(B * N, 1) * self.noise_high).to(self.device)
        ...
        lr = torch.clamp(lr, 0, 255.).round().type(torch.uint8).float()
        lr_n = torch.clamp(lr_n, 0, 255.).round().type(torch.uint8).float()

        return (lr.view(B, N, C, H//int(self.scale), W//int(self.scale)),
                lr_n.view(B, N, C, H//int(self.scale), W//int(self.scale)), b_sigmas, b_kernels) \
            if kernel else \
            (lr.view(B, N, C, H//int(self.scale), W//int(self.scale)),
             lr_n.view(B, N, C, H//int(self.scale), W//int(self.scale)), b_kernels)
...
if __name__ == '__main__':
    H, W = 128, 128
    divide_H, divide_W = 40, 40
    B, N = 1, 2
    tensor = torch.rand(B* N, 3, 128, 128)
    block_H = math.ceil(H / divide_H)  # patch pos
    block_W = math.ceil(W / divide_W)
    Noise_level = torch.FloatTensor(random_batch_noise(B*N*block_H*block_W, 5, 0.2))
    x_n = bp_GaussianNoising(tensor, Noise_level, divide_P=40)